...
@Component(value = ContainerType.Value.INFO)
public class InfoContainerGenerator extends AbstractContainerGenerator implements ContainerDataGenerator {
    ...
    public List<SectionField> generate(DataContext dataContext) throws DataException {
        ...
        if(PlaceHolderType.PICK_FROM_COMMON_QUERY.value().equals(queryString)) {
           dbData = fetchCommonPageData(dataContext);
        }
        else {
            dbData = fetchSectionData(dataContext);
        }
        if(!CollectionUtils.isEmpty(dbData) && dbData.size() > 1) {
            dbData = dbData.subList(0,1);
        }
