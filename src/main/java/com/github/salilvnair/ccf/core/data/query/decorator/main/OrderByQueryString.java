...
public class OrderByQueryString extends AbstractQueryString {

    public OrderByQueryString() {
        super();
    }
    public OrderByQueryString(QueryString queryString) {
        super(queryString);
    }
    ...
    public String replaceWith(DataContext dataContext) {
        ...
        if(dataContext.isSort() && !CollectionUtils.isEmpty(sortParams)
                && !sortParams.stream().allMatch(sortParam -> sortParam.getFieldId() == null)) {
