...
public class AnnotationUtil {
	...
	public static Set<Annotation> extractAllAnnotations(final Class<?> cls) {
		...
		for (Annotation ann : anns) {
			extractSuperAnnotations(ann.annotationType(), superAnnotations);
		}
	...
	private static <A extends Annotation> void extractSuperAnnotations(Class<A> annotationType, Set<Annotation> visited) {
		Annotation[] anns = annotationType.getDeclaredAnnotations();

		for (Annotation ann : anns) {
			if (!ann.annotationType().getName().startsWith("java.lang") && visited.add(ann)) {
				extractSuperAnnotations(ann.annotationType(), visited);
			}
		}
	}

	public static <T extends Annotation> Set<Field> extractAnnotatedPublicFields(Class<?> clazz,
																				 Class<T> annotation) {
	...
	public static <T extends Annotation> Set<Field> extractAnnotatedFields(Class<?> clazz,
																		   Class<T> annotation) {
		if (Object.class.equals(clazz)) {
			return Collections.emptySet();
		}
		...
		annotatedFields.addAll(extractAnnotatedFields(clazz.getSuperclass(), annotation));
		return annotatedFields;
	}


	public static <T extends Annotation> Set<Method> extractAnnotatedPublicMethods(Class<?> clazz, Class<T> annotation) {
		if (Object.class.equals(clazz)) {
			return Collections.emptySet();
		}
